// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from hospital_interfaces:srv/GetPatrolContext.idl
// generated code does not contain a copyright notice
#include "hospital_interfaces/srv/detail/get_patrol_context__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"

bool
hospital_interfaces__srv__GetPatrolContext_Request__init(hospital_interfaces__srv__GetPatrolContext_Request * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
hospital_interfaces__srv__GetPatrolContext_Request__fini(hospital_interfaces__srv__GetPatrolContext_Request * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
hospital_interfaces__srv__GetPatrolContext_Request__are_equal(const hospital_interfaces__srv__GetPatrolContext_Request * lhs, const hospital_interfaces__srv__GetPatrolContext_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
hospital_interfaces__srv__GetPatrolContext_Request__copy(
  const hospital_interfaces__srv__GetPatrolContext_Request * input,
  hospital_interfaces__srv__GetPatrolContext_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

hospital_interfaces__srv__GetPatrolContext_Request *
hospital_interfaces__srv__GetPatrolContext_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hospital_interfaces__srv__GetPatrolContext_Request * msg = (hospital_interfaces__srv__GetPatrolContext_Request *)allocator.allocate(sizeof(hospital_interfaces__srv__GetPatrolContext_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(hospital_interfaces__srv__GetPatrolContext_Request));
  bool success = hospital_interfaces__srv__GetPatrolContext_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
hospital_interfaces__srv__GetPatrolContext_Request__destroy(hospital_interfaces__srv__GetPatrolContext_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    hospital_interfaces__srv__GetPatrolContext_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
hospital_interfaces__srv__GetPatrolContext_Request__Sequence__init(hospital_interfaces__srv__GetPatrolContext_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hospital_interfaces__srv__GetPatrolContext_Request * data = NULL;

  if (size) {
    data = (hospital_interfaces__srv__GetPatrolContext_Request *)allocator.zero_allocate(size, sizeof(hospital_interfaces__srv__GetPatrolContext_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = hospital_interfaces__srv__GetPatrolContext_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        hospital_interfaces__srv__GetPatrolContext_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
hospital_interfaces__srv__GetPatrolContext_Request__Sequence__fini(hospital_interfaces__srv__GetPatrolContext_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      hospital_interfaces__srv__GetPatrolContext_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

hospital_interfaces__srv__GetPatrolContext_Request__Sequence *
hospital_interfaces__srv__GetPatrolContext_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hospital_interfaces__srv__GetPatrolContext_Request__Sequence * array = (hospital_interfaces__srv__GetPatrolContext_Request__Sequence *)allocator.allocate(sizeof(hospital_interfaces__srv__GetPatrolContext_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = hospital_interfaces__srv__GetPatrolContext_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
hospital_interfaces__srv__GetPatrolContext_Request__Sequence__destroy(hospital_interfaces__srv__GetPatrolContext_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    hospital_interfaces__srv__GetPatrolContext_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
hospital_interfaces__srv__GetPatrolContext_Request__Sequence__are_equal(const hospital_interfaces__srv__GetPatrolContext_Request__Sequence * lhs, const hospital_interfaces__srv__GetPatrolContext_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!hospital_interfaces__srv__GetPatrolContext_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
hospital_interfaces__srv__GetPatrolContext_Request__Sequence__copy(
  const hospital_interfaces__srv__GetPatrolContext_Request__Sequence * input,
  hospital_interfaces__srv__GetPatrolContext_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(hospital_interfaces__srv__GetPatrolContext_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    hospital_interfaces__srv__GetPatrolContext_Request * data =
      (hospital_interfaces__srv__GetPatrolContext_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hospital_interfaces__srv__GetPatrolContext_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          hospital_interfaces__srv__GetPatrolContext_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!hospital_interfaces__srv__GetPatrolContext_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `global_context`
// Member `final_summary`
#include "rosidl_runtime_c/string_functions.h"

bool
hospital_interfaces__srv__GetPatrolContext_Response__init(hospital_interfaces__srv__GetPatrolContext_Response * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // global_context
  if (!rosidl_runtime_c__String__init(&msg->global_context)) {
    hospital_interfaces__srv__GetPatrolContext_Response__fini(msg);
    return false;
  }
  // final_summary
  if (!rosidl_runtime_c__String__init(&msg->final_summary)) {
    hospital_interfaces__srv__GetPatrolContext_Response__fini(msg);
    return false;
  }
  return true;
}

void
hospital_interfaces__srv__GetPatrolContext_Response__fini(hospital_interfaces__srv__GetPatrolContext_Response * msg)
{
  if (!msg) {
    return;
  }
  // success
  // global_context
  rosidl_runtime_c__String__fini(&msg->global_context);
  // final_summary
  rosidl_runtime_c__String__fini(&msg->final_summary);
}

bool
hospital_interfaces__srv__GetPatrolContext_Response__are_equal(const hospital_interfaces__srv__GetPatrolContext_Response * lhs, const hospital_interfaces__srv__GetPatrolContext_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // global_context
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->global_context), &(rhs->global_context)))
  {
    return false;
  }
  // final_summary
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->final_summary), &(rhs->final_summary)))
  {
    return false;
  }
  return true;
}

bool
hospital_interfaces__srv__GetPatrolContext_Response__copy(
  const hospital_interfaces__srv__GetPatrolContext_Response * input,
  hospital_interfaces__srv__GetPatrolContext_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // global_context
  if (!rosidl_runtime_c__String__copy(
      &(input->global_context), &(output->global_context)))
  {
    return false;
  }
  // final_summary
  if (!rosidl_runtime_c__String__copy(
      &(input->final_summary), &(output->final_summary)))
  {
    return false;
  }
  return true;
}

hospital_interfaces__srv__GetPatrolContext_Response *
hospital_interfaces__srv__GetPatrolContext_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hospital_interfaces__srv__GetPatrolContext_Response * msg = (hospital_interfaces__srv__GetPatrolContext_Response *)allocator.allocate(sizeof(hospital_interfaces__srv__GetPatrolContext_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(hospital_interfaces__srv__GetPatrolContext_Response));
  bool success = hospital_interfaces__srv__GetPatrolContext_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
hospital_interfaces__srv__GetPatrolContext_Response__destroy(hospital_interfaces__srv__GetPatrolContext_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    hospital_interfaces__srv__GetPatrolContext_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
hospital_interfaces__srv__GetPatrolContext_Response__Sequence__init(hospital_interfaces__srv__GetPatrolContext_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hospital_interfaces__srv__GetPatrolContext_Response * data = NULL;

  if (size) {
    data = (hospital_interfaces__srv__GetPatrolContext_Response *)allocator.zero_allocate(size, sizeof(hospital_interfaces__srv__GetPatrolContext_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = hospital_interfaces__srv__GetPatrolContext_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        hospital_interfaces__srv__GetPatrolContext_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
hospital_interfaces__srv__GetPatrolContext_Response__Sequence__fini(hospital_interfaces__srv__GetPatrolContext_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      hospital_interfaces__srv__GetPatrolContext_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

hospital_interfaces__srv__GetPatrolContext_Response__Sequence *
hospital_interfaces__srv__GetPatrolContext_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  hospital_interfaces__srv__GetPatrolContext_Response__Sequence * array = (hospital_interfaces__srv__GetPatrolContext_Response__Sequence *)allocator.allocate(sizeof(hospital_interfaces__srv__GetPatrolContext_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = hospital_interfaces__srv__GetPatrolContext_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
hospital_interfaces__srv__GetPatrolContext_Response__Sequence__destroy(hospital_interfaces__srv__GetPatrolContext_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    hospital_interfaces__srv__GetPatrolContext_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
hospital_interfaces__srv__GetPatrolContext_Response__Sequence__are_equal(const hospital_interfaces__srv__GetPatrolContext_Response__Sequence * lhs, const hospital_interfaces__srv__GetPatrolContext_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!hospital_interfaces__srv__GetPatrolContext_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
hospital_interfaces__srv__GetPatrolContext_Response__Sequence__copy(
  const hospital_interfaces__srv__GetPatrolContext_Response__Sequence * input,
  hospital_interfaces__srv__GetPatrolContext_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(hospital_interfaces__srv__GetPatrolContext_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    hospital_interfaces__srv__GetPatrolContext_Response * data =
      (hospital_interfaces__srv__GetPatrolContext_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!hospital_interfaces__srv__GetPatrolContext_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          hospital_interfaces__srv__GetPatrolContext_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!hospital_interfaces__srv__GetPatrolContext_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
