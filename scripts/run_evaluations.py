...
# Comando base común para el evaluador (Asegurando el parámetro use_reranker:=true en TODOS)
...
TESTS = [
    ...
    # ----------- 1. Híbrido VS Solo VLM (Dataset Grande) -----------
    {
    ...
    # ----------- 2. Estático (VLM) VS Vídeo (Nuevo Estándar) -----------
    {
        "category": "3_imagen_vs_video",
        "name": "3_eval_vlm_static",
        "dataset": "reduccion_1_0m", 
        # Como ahora va antes, DEBE ejecutar los comandos (ya no copia de reducciones)
        "perception_cmd": "ros2 run ruta_hospital hybrid_perception_node",
        "eval_cmd": f"{BASE_EVAL_CMD} -p evaluation_name:=\"3_eval_vlm_static\" -p perception_mode:=\"image\"",
        "skip_and_copy_from": None
    },
    {
        "category": "3_imagen_vs_video",
    ...
    # Reducciones de Tamaño
    {
        ...
        "skip_and_copy_from": {
            "category": "3_imagen_vs_video",
            "name": "3_eval_vlm_static" # <-- Ahora es esta prueba la que ahorra horas clonando del paso 2
        }
    },
    {
        "category": "2_size_datasets",
    ...
    # Reducciones de MSE
    {
        "category": "5_mse_datasets",
        "name": "5_eval_dataset_mse_00_5",
        "dataset": "mse_00_5",
        "perception_cmd": "ros2 run ruta_hospital hybrid_perception_node",
        "eval_cmd": f"{BASE_EVAL_CMD} -p evaluation_name:=\"5_eval_dataset_mse_00_5\" -p perception_mode:=\"image\"",
        "skip_and_copy_from": None
    },
    {
        "category": "5_mse_datasets",
        "name": "5_eval_dataset_mse_01",
        "dataset": "mse_01",
        "perception_cmd": "ros2 run ruta_hospital hybrid_perception_node",
        "eval_cmd": f"{BASE_EVAL_CMD} -p evaluation_name:=\"5_eval_dataset_mse_01\" -p perception_mode:=\"image\"",
        "skip_and_copy_from": None
    },
    {
        "category": "5_mse_datasets",
        "name": "5_eval_dataset_mse_05",
        "dataset": "mse_05",
        "perception_cmd": "ros2 run ruta_hospital hybrid_perception_node",
        "eval_cmd": f"{BASE_EVAL_CMD} -p evaluation_name:=\"5_eval_dataset_mse_05\" -p perception_mode:=\"image\"",
        "skip_and_copy_from": None
    },
    {
        "category": "5_mse_datasets",
        "name": "5_eval_dataset_grande",
        "dataset": "grande",
        "perception_cmd": None,
        "eval_cmd": None,
        "skip_and_copy_from": {
    ...
    # ----------- 4. Híbrido Normal con Reportero Qwen3.5:4b -----------
    {
...
def clean_and_restore_faiss(test):
    """Fuerza la copia absoluta del directorio FAISS histórico hacia /tmp. 
    Si el experimento está en modo evaluate_only, también restaura las respuestas JSON.
    Si detecta resume_session:=false, limpia el entorno sin restaurar índices viejos."""
    ...
    # Esperar a que el S.O libere los handles del borrado
    time.sleep(1)
        
    # 1. Comprobar si este experimento exige regeneración limpia desde cero
    if test.get("eval_cmd") and '-p resume_session:=false' in test["eval_cmd"]:
        print("   [FAISS] Forzando regeneración completa solicitada por el test. Saltando restauración de índice anterior.")
    else:
        # Restaurar Base Vectorial FAISS ordinaria si existe caché previa
        ...
        if os.path.exists(archive_rag_data):
            print(f"   [FAISS] Restaurando índice de {archive_rag_data} hacia /tmp...")
            shutil.copytree(archive_rag_data, TMP_RAG_DATA)
        else:
            print(f"   [FAISS] No se encontró caché FAISS en {archive_rag_data}.")

    # 2. Restaurar el archivo JSON si es 'evaluate_only' o una generación parcial ('summary_only')
    is_evaluate_only = 'evaluation_mode:="evaluate_only"' in test.get("eval_cmd", "")
    is_partial_generation = 'evaluation_target:="summary_only"' in test.get("eval_cmd", "") or 'evaluation_target:="short_only"' in test.get("eval_cmd", "")
    
    if is_evaluate_only or is_partial_generation:
        archive_answers = os.path.join(TMP_ARCHIVE_BASE_DIR, test["category"], test["name"], "ragas_intermediate_answers.json")
        if os.path.exists(archive_answers):
            print(f"   [RESPUESTAS] Restaurando respuestas previas desde {archive_answers} hacia /tmp...")
            shutil.copy2(archive_answers, TMP_RAGAS_ANSWERS)
        else:
            print(f"   [RESPUESTAS] ⚠️ ADVERTENCIA: No se encontró el archivo JSON en {archive_answers}.")
    else:
        print(f"   [RESPUESTAS] Modo Full/Generación detectado. Dejando /tmp limpio para respuestas nuevas.")
        
    time.sleep(2) # Dar tiempo para que los archivos se asienten en el sistema de archivos
...
def run_pipeline():
    ...
    while len(completed_tests) < len(TESTS):
        ...
        # Llamar explícitamente a la restauración antes de arrancar los nodos
        clean_and_restore_faiss(current_test)

        # Si el test requiere perceptor, levantarlo
        if current_test.get("perception_cmd") is not None:
            print("[ROS2] Desplegando nodo de Percepción...")
            spawn_terminal(f"Perception Node: {current_test['name']}", current_test["perception_cmd"])
            time.sleep(4)
        else:
            print("[ROS2] Modo Re-evaluación: Perceptor anulado para no machacar FAISS.")

        print("[ROS2] Desplegando nodo Evaluador...")
        spawn_terminal(f"System Evaluator: {current_test['name']}", current_test["eval_cmd"])
        time.sleep(5)

        print("[ROS2] Enviando Goal de acción...")
        ...
        print(f"\n[ROS2] Goal completado. Limpiando nodos...")
        kill_ros_nodes()
        time.sleep(2)
